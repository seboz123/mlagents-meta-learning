...
class PPOOptimizer(TFOptimizer):
    def __init__(self, policy: TFPolicy, trainer_params: TrainerSettings):
        ...
        with policy.graph.as_default():
            with tf.variable_scope("optimizer/"):
                ...
                # self._schedule = ScheduleType.CONSTANT # Set Constant Schedule for meta learning
    ...
    def _create_losses(
        ...
        self.decay_epsilon = ModelUtils.create_schedule(
            self._schedule, epsilon, self.policy.global_step, max_step, min_value=epsilon
        )
        self.decay_beta = ModelUtils.create_schedule(
            self._schedule, beta, self.policy.global_step, max_step, min_value=beta
        )
        ...
        # print(self.decay_beta.eval(session=self.sess))
        self.loss = (
            self.policy_loss
            + 0.5 * self.value_loss
            - self.decay_beta
