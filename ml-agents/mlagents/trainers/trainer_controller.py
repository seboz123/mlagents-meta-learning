...
class TrainerController(object):
    ...
    def init_weights(self, env_manager):
        ...
        self.weights = deepcopy(self.trainers['Brain'].get_policy(0).get_weights())
        ...
        try:
            ...
            while self._not_done_training():
                ...
                if(inital_weights is not None and first_step):
                    ...
                    # Set weights
                    with self.trainers['Brain'].get_policy(0).graph.as_default():
                        _vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
                        values = [v.eval(session=self.trainers['Brain'].get_policy(0).sess) for v in _vars]
                        ...
                        for var, value in zip(_vars, values):
                            ...
                            self.trainers['Brain'].get_policy(0).assign_phs.append(assign_ph)
                            self.trainers['Brain'].get_policy(0).assign_ops.append(tf.assign(var, assign_ph))
                        # print(self.trainers['Brain'].get_policy(0).assign_ops)
                        # print(self.trainers['Brain'].get_policy(0).assign_phs)
                    self.trainers['Brain'].get_policy(0).load_weights(inital_weights)
